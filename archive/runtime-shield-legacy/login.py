...
def run_login():
    ...
    print("\nSelect login method:")
    print("1. Browser Login — Keycloak (Authorization Code + PKCE — Recommended)")
    print("2. Google Login via Keycloak (Browser — Recommended)")
    print("3. Manual Login (Password Grant — DEV ONLY)" + ("" if is_dev else " [DISABLED — set LOCAL_DEV_MODE=true]"))
    print("4. Mock Login (Demo Mode — DEV ONLY)" + ("" if is_dev else " [DISABLED — set LOCAL_DEV_MODE=true]"))
    ...
    if choice == "2":
        # Google Login — redirect directly to Google via Keycloak identity brokering
        _browser_login(idp_hint="google")
        return

    # Default: choice == "1" — standard Keycloak browser login (no idp_hint)
    _browser_login()


def _browser_login(idp_hint: str = None):
    """Core browser-based Authorization Code flow.

    Args:
        idp_hint: Optional Keycloak IDP alias (e.g. 'google') to skip Keycloak's own
                  login page and go directly to that identity provider. When None the
                  normal Keycloak login page is shown.
    """
    global auth_code
    auth_code = None  # reset for each call

    # Check if the callback port is free
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        if s.connect_ex(('localhost', 18080)) == 0:
            print("Error: Port 18080 is already in use. Close whatever is occupying it and try again.")
            return
    ...
    if idp_hint == "google":
        print(f"\n🌐 Redirecting to Google Login via Keycloak...")
        print(f"   Keycloak will broker your Google identity and return a standard Keycloak token.")
    else:
        print(f"\n🔑 Opening Keycloak login in browser...")
    print(f"   URL: {full_url}")
    webbrowser.open(full_url)

    print("\n⏳ Waiting for callback on port 18080... (Press Ctrl+C to abort)")
    try:
        while auth_code is None:
            server.handle_request()

        if auth_code:
            print("✅ Code received! Exchanging for Keycloak token...")
            token_url = urljoin(KEYCLOAK_URL, f"realms/{REALM}/protocol/openid-connect/token")
            data = {
